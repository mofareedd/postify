      ...
      {Object.values(authOptions.providers).map((provider) => {
        ...
        return (
          <div key={provider.name} className="w-full">
            ...
            >
              {provider.id === 'github' && !isLoading ? <Icons.gitHub className="mr-2 h-4 w-4" aria-hidden="true" /> : null}
              {provider.id === 'google' ? <Icons.google className="mr-2 h-4 w-4" aria-hidden="true" /> : null}
              {provider.name}
            </Button>
          </div>
        );
